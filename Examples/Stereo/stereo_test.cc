...
void LoadImages(const string &pathtosettings, const string &strPathToSequence, vector<string> &vstrImageLeft,
                vector<string> &vstrImageRight, vector<string> &vstrImagemaskleft, vector<double> &vTimestamps);
...
int main(int argc, char **argv) {
    ...
    vector<string> vstrImagemaskleft;
    vector<double> vTimestamps;
    //　导入mask 图片
    LoadImages( string(argv[2]), string(argv[3]), vstrImageLeft, vstrImageRight, vstrImagemaskleft, vTimestamps);
    const int nImages = vstrImageLeft.size();
    // Ncc找ＲＯＩ
    ORB_SLAM3::Nccfind(argv[2], vstrImageLeft, vstrImageRight);
    ORB_SLAM3::readcameraMat(argv[2]);
    //  记录匹配数
    std::string sstr = "../../my_testlog/matchednums.txt";
    ...
    // Main loop
    cv::Mat imLeft, imRight, immaskleft;
    for(int ni = 0; ni < nImages; ni++) {
        ORB_SLAM3::squenceimg = ni;
        // Read left and right images from file
        imLeft = cv::imread(vstrImageLeft[ni], cv::IMREAD_UNCHANGED);
        imRight = cv::imread(vstrImageRight[ni], cv::IMREAD_UNCHANGED);
        immaskleft = cv::imread(vstrImagemaskleft[ni], cv::IMREAD_UNCHANGED);
        ...
        cv::Mat TrackT = SLAM.TrackStereo(imLeft, imRight, immaskleft, tframe, vstrImageLeft[ni]) ;

        // 写入匹配数的数据，之前的改进双目金字塔匹配数量对比
        fout << ORB_SLAM3::objbestdistance << endl;
...
void LoadImages(const string &pathtosettings, const string &strPathToSequence, vector<string> &vstrImageLeft,
    ...
    string strPrefixLeft, strPrefixRight, strPrefixmask;
    ...
    if(cameratype == "01") {
        ...
        strPrefixmask = strPathToSequence + "/mask_img0/";
        ...
        strPrefixRight = strPathToSequence + "/image_2/";
        strPrefixmask = strPathToSequence + "/mask_img1/";
    ...
    vstrImagemaskleft.resize(nTimes);
    for(int i = 0; i < nTimes; i++) {
        stringstream ss;
        ss << setfill('0') << setw(6) << i;
        vstrImageLeft[i] = strPrefixLeft + ss.str() + ".png";
        vstrImageRight[i] = strPrefixRight + ss.str() + ".png";
        vstrImagemaskleft[i] = strPrefixmask + ss.str() + ".png";
    }
}
