...
// 前后帧之间只匹配分割区域
    /**
     * @brief ORBmatcher::MFSearchForSegmentregion
     * @param[in] F1                        前一帧
     * @param[in] F2                        当前帧
     * @param[in & out] vbPrevMatched       本来存储的是参考帧的所有特征点坐标，该函数更新为匹配好的当前帧的特征点坐标
     * @param[in & out] vnMatches12         保存参考帧F1中特征点是否匹配上，index保存是F1对应特征点索引，值保存的是匹配好的F2特征点索引
     * @param[in] windowSize                搜索窗口 原设定值维100
     */
    void ORBmatcher::MFSearchForSegmentregion(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<float> &vnMatches12, vector<float> &vnoutMatches12, int windowSize) {
        int nmatches = 0;
        // F1中特征点和F2中匹配关系，注意是按照F1特征点数目分配空间
        vnMatches12 = vector<float>(F1.mvKeysUn.size(), -1);
        vnoutMatches12 = vector<float>(F1.mvKeysUn.size(), -1);
        vector<float> vnoutMatches21 = vector<float>(F2.mvKeysUn.size(), -1);
        ...
        //! 原作者代码是 const float factor = 1.0f/HISTO_LENGTH;  小刘等人认为　const float factor = HISTO_LENGTH/360.0f;
        const float factor = 1.0f / HISTO_LENGTH;
        // 匹配点对距离，注意是按照F2特征点数目分配空间 mask内
        vector<int> vMatchedDistance(F2.mvKeysUn.size(), INT_MAX);
        ...
        for(size_t i1 = 0, iend1 = F1.mvKeysUn.size(); i1 < iend1; i1++) {
            ...
            bool whetherinmask = false;

            cv::KeyPoint kp1 = F1.mvKeysUn[i1];
            int level1 = kp1.octave;
            int minlevel, maxlevel;
            // 7是测试数据
            minlevel = max(0, level1 - 2);
            maxlevel = min((int)ORB_SLAM3::MFnLevels - 1, level1 + 2);
            // 只使用原始图像上提取的特征点
            // Step 2 在半径窗口内搜索当前帧F2中所有的候选匹配特征点
            // vbPrevMatched 输入的是参考帧 F1的特征点
            // windowSize = 100，输入最大最小金字塔层级 均为0
            // GetFeaturesInArea 找到在 以x,y为中心,半径为r的圆形内且金字塔层级在[minLevel, maxLevel]的特征点
            vector<size_t> vIndices2 = F2.GetFeaturesInArea(vbPrevMatched[i1].x, vbPrevMatched[i1].y, windowSize, minlevel, maxlevel);
            // 没有候选特征点，跳过
            if(vIndices2.empty()) {
                continue;
            }
            ...
            for(vector<size_t>::iterator vit = vIndices2.begin(); vit != vIndices2.end(); vit++) {
                size_t i2 = *vit;
                cv::KeyPoint delkp2 = F2.mvKeysUn[i2];
                if((int)F2.maskleft.at<unsigned char>(round(delkp2.pt.y), round(delkp2.pt.x)) == 0 &&
                        (int)F1.maskleft.at<unsigned char>(round(kp1.pt.y), round(kp1.pt.x)) == 0) {
                    whetherinmask = true;
//                    continue;
                }
                // 取出候选特征点对应的描述子
            ...
            if(bestDist <= TH_LOW) {
                ...
                if(bestDist < (float)bestDist2 * mfNNratio) {
                    ...
                    if(whetherinmask) {
                        vnoutMatches12[i1] = bestIdx2;
                        vnoutMatches21[bestIdx2] = i1;
                    } else {
                        vnMatches12[i1] = bestIdx2;
                        vnMatches21[bestIdx2] = i1;
                        vMatchedDistance[bestIdx2] = bestDist;
        ...
        if(mbCheckOrientation) {
            ...
            for(int i = 0; i < HISTO_LENGTH; i++) {
                ...
                for(size_t j = 0, jend = rotHist[i].size(); j < jend; j++) {
                    ...
                    if(vnoutMatches12[idx1] >= 0) {
                        vnoutMatches12[idx1] = -1;
                    }
                }
            }
        }
    ...
    /**
     * @brief ORBmatcher::MFSearchForSegmentregiondyna
     * @param F1        前一帧
     * @param F2        后一帧
     * @param vbPrevMatched
     * @param vnMatches12   前一阵和当前帧匹配关系，全部点
     * @param vnoutMatches12   暂时不用，
     * @param windowSize
     */
    void ORBmatcher::MFSearchForSegmentregiondyna(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<float> &vnMatches12, vector<float> &vnoutMatches12, int windowSize) {
        int nmatches = 0;
        // F1中特征点和F2中匹配关系，注意是按照F1特征点数目分配空间，所有点
        vnMatches12 = vector<float>(F1.mvKeysUndyna.size(), -1);
        // 匹配掩码外的点，用于计算H矩阵
        //　当前帧对前一帧的匹配
        // Step 1 构建旋转直方图，HISTO_LENGTH = 30
        vector<int> rotHist[HISTO_LENGTH];
        ...
        const float factor = 1.0f / HISTO_LENGTH;
        // 匹配点对距离，注意是按照F2特征点数目分配空间 mask内
        vector<int> vMatchedDistance(F2.mvKeysUndyna.size(), INT_MAX);

        // 匹配点对距离，注意是按照F2特征点数目分配空间 mask外
        // 从帧2到帧1的反向匹配，注意是按照F2特征点数目分配空间
        vector<int> vnMatches21(F2.mvKeysUndyna.size(), -1);
        for(size_t i1 = 0, iend1 = F1.mvKeysUndyna.size(); i1 < iend1; i1++) {

            cv::KeyPoint kp1 = F1.mvKeysUndyna[i1];
            int level1 = kp1.octave;
            int minlevel, maxlevel;
            // 7是测试数据
            minlevel = max(0, level1 - 2);
            ...
            // 取出参考帧F1中当前遍历特征点对应的描述子
            //BUG
            int mDescriptorsrows = F1.mDescriptors.rows;
            if(i1 >= mDescriptorsrows) {
//                throw(i1, mDescriptorsrows);
                continue;
//                break;
            }
            cv::Mat d1 = F1.mDescriptors.row(i1);
            ...
            for(vector<size_t>::iterator vit = vIndices2.begin(); vit != vIndices2.end(); vit++) {
                size_t i2 = *vit;
                cv::KeyPoint delkp2 = F2.mvKeysUndyna[i2];
                // 取出候选特征点对应的描述子
                cv::Mat d2 = F2.mDescriptors.row(i2);
                // 计算两个特征点描述子距离
                int dist = DescriptorDistance(d1, d2);
            ...
            if(bestDist <= TH_LOW) {
                ...
                if(bestDist < (float)bestDist2 * mfNNratio) {
                    ...
                    if(mbCheckOrientation) {
                        // 计算匹配特征点的角度差，这里单位是角度°，不是弧度
                        float rot = F1.mvKeysUndyna[i1].angle - F2.mvKeysUndyna[bestIdx2].angle;
                        if(rot < 0.0) {
                            rot += 360.0f;
                        }
                        // 前面factor = HISTO_LENGTH/360.0f
        ...
        for(size_t i1 = 0, iend1 = vnMatches12.size(); i1 < iend1; i1++)
            if(vnMatches12[i1] >= 0) {
                vbPrevMatched[i1] = F2.mvKeysUndyna[vnMatches12[i1]].pt;
            }
    }
            ...
            if(pMP) {
                if(!LastFrame.mvbOutlier[i]) {
                    ...
                    for(vector<size_t>::const_iterator vit = vIndices2.begin(), vend = vIndices2.end(); vit != vend; vit++) {
                        ...
                        //BUG
                        if(i2 <= 0 || i2 >= CurrentFrame.mDescriptors.rows) {
                            break;
//                            continue;
                        }
