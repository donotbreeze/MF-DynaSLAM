    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false)
{
    ...
    cout << endl <<
    "MF-DynaSLAM, 2023 Mingchi Feng, Xuan Yi, Chengnan Li, Jia Zheng." << endl <<
...
cv::Mat System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const cv::Mat &immaskleft, const double &timestamp, string filename, const vector<IMU::Point>& vImuMeas)
{
    if(mSensor!=STEREO && mSensor!=IMU_STEREO)
    {
        cerr << "ERROR: you called TrackStereo but input sensor was not set to Stereo nor Stereo-Inertial." << endl;
    ...
    if (mSensor == System::IMU_STEREO)
        for(size_t i_imu = 0; i_imu < vImuMeas.size(); i_imu++)
            mpTracker->GrabImuData(vImuMeas[i_imu]);
    cv::Mat Tcw = mpTracker->GrabImageStereo(imLeft,imRight,immaskleft,timestamp,filename);
    unique_lock<mutex> lock2(mMutexState);
    mTrackingState = mpTracker->mState;
    mTrackedMapPoints = mpTracker->mCurrentFrame.mvpMapPoints;
    mTrackedKeyPointsUn = mpTracker->mCurrentFrame.mvKeysUn;
